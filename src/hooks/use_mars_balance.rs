...
use crate::gateway::{mars_token_account_address, AsyncResult, GatewayError};
...
pub fn use_mars_balance_provider(cx: &ScopeState) {
    // Balance state.
...
pub fn use_mars_balance_handle(cx: &ScopeState) -> BalanceHandle {
    cx.consume_context::<BalanceHandle>().unwrap()
}

pub fn use_mars_balance(cx: &ScopeState) -> AsyncResult<UiTokenAmount> {
...
pub fn use_mars_balance_user(cx: &ScopeState, pubkey: Pubkey) -> AsyncResult<UiTokenAmount> {
    // Balance state.
    let gateway = use_gateway(cx);
    let token_account_address = mars_token_account_address(pubkey);
