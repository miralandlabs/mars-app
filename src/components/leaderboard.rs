...
use crate::{
    ...
    hooks::{use_gateway, use_mars_supply, use_treasury},
    route::Route,
};
        ...
        div {
            ...
            div {
                ...
                MarsValue {
                    ...
                    detail: "The total amount of Mars that has been mined and claimed.".to_string(),
                    amount: circulating_supply
                }
                MarsValue {
                    title: "Total supply".to_string(),
                    detail: "The total amount of Mars that has ever been mined.".to_string(),
                    amount: mars_supply
                }
            }
        }
...
#[component]
fn MarsValue(cx: Scope, title: String, detail: String, amount: String) -> Element {
    render! {
        div {
...
pub fn TokenBalanceRow<'a>(cx: Scope, i: usize, token_account: &'a UiTokenAccount) -> Element {
    let owner = if token_account.owner.eq(&mars::TREASURY_ADDRESS.to_string()) {
        "Mars Treasury".to_string()
    } else {
...
async fn fetch_top_accounts(gateway: Rc<Gateway>) -> Vec<UiTokenAccount> {
    ...
    if let Ok(balances) = gateway.get_token_largest_accounts(&mars::MINT_ADDRESS).await {
